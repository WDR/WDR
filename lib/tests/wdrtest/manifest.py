...
class VariableSubstitutionTest( unittest.TestCase ):
    ...
    def testWithNestedDictionary( self ):
        self.assertEquals( 'Hello John Peter Smith!', substituteVariables( 'Hello $[person.name.first] $[person.name.second] $[person.name.last]!', { 'person': { 'name': { 'first':'John', 'second':'Peter', 'last':'Smith' } } } ) )
    def testNone( self ):
    ...
    def testCallable( self ):
        self.assertEquals( 'Hello world!', substituteVariables( 'Hello $[name]!', {'name': lambda expression, variables: 'world'} ) )
    def testCallableReturningNone( self ):
        self.assertEquals( 'Hello !', substituteVariables( 'Hello $[name]!', {'name': lambda expression, variables: None} ) )
    def testWithExtraSpaces( self ):
        self.assertEquals( 'Hello world!', substituteVariables( 'Hello $[ person.name ]!', {'person': {'name': 'world'}} ) )

class VariableSubstitutionWithFilteringTest( unittest.TestCase ):
    def testUpper( self ):
        self.assertEquals( 'Hello WORLD!', substituteVariables( 'Hello $[name|upper]!', {'name': 'world', 'upper': string.upper } ) )
    def testListProcessing( self ):
        self.assertEquals( 'Hello John Smith!', substituteVariables( 'Hello $[name|join]!', {'name': ['John', 'Smith'], 'join': string.join } ) )
    def testListWithLambdaOnList( self ):
        self.assertEquals( 'deploymentTargets are [WebSphere:cell=wdrCell,cluster=wdrCluster+WebSphere:cell=wdrCell,node=httpNode01,server=httpServer01]',
            substituteVariables( 'deploymentTargets are [$[deploymentTargets|joinDeploymentTargets]]',
                {
                    'deploymentTargets': [
