...
if [ "${TARGET_ENV}" == "" ]; then
	...
	if [ "${SCRIPT}" == "" ]; then
		${JAVA_HOME}/bin/java ${WSADMIN_TRACE_STRING} ${USER_ROOT_PROPERTY} ${PYTHON_PATH} ${JYTHON_CACHEDIR} ${TMPDIR_PROPERTY} ${WSADMIN_EXCEPTION_PROPAGATION} ${WDR_PROFILE_PROPERTY} ${SOAP_PROPERTIES_PROPERTY} ${SSL_CLIENT_PROPS_PROPERTY} ${WAS_DEBUG} ${CONSOLE_ENCODING_PROPERTY} $ {APP_TIMEOUT_PROPERTY} ${WAS_LOGGING_PROPERTIES} ${THIN_CLIENT_PROPERTY} ${PERFJAVAOPTIONS} ${WSADMIN_PROPERTIES_PROPERTY} -Duser.install.root=${WAS_HOME} -Dwas.install.root=${WAS_HOME} -cp ${WSADMIN_CLASS_PATH} ${SHELL} $*
	else
